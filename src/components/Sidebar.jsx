...
// Sidebar Menu Data (Keep this array consistent)
const menuItems = [
...
// Receive the activeView state and its setter function
const Sidebar = ({ onOpenContext, activeView, setActiveView }) => {
    
    // The handleNavigation logic now just calls the function passed from App.jsx
    ...
    return (
        <div className="fixed top-0 left-0 h-screen w-20 p-4 z-50 
                ...
                {menuItems.map((item) => (
                    <button 
                        ...
                        className={`p-3 rounded-xl transition-all duration-200 
                                    ${activeView === item.name // Check against the external state
            ...
            {/* Context Viewer Button (remains the same) */}
            <div className="mb-4">
                <button
